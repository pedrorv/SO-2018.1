...
class Blockchain {
    ...
    return chain.every((block, index) => {
      ...
      if (!listBlocks.includes(block.hash)){
        ...
        transactions.forEach(transaction => {
          if (transaction.input.amount != newBlockChain.getBalance(transaction.input.address), transaction.input.timestamp)
            return false;
          if (transaction.input.amount != transaction.outputs.reduce((amt, o) => amt += o.amount, 0 ))
            return false;
        });
      }
  ...
  replaceChain(newChain) {
    const actualChainDifficulty = this.chain.reduce((acc, block) => acc += Math.pow(2,block.difficulty), 0);
    const newChainDifficulty = this.chain.reduce((acc, block) => acc += Math.pow(2,block.difficulty), 0);
  ...
  getBalance( address, currenttimestamp = new Date()) {
    ...
    if (walletInputs.length) {
      ...
      balance = mostRecentInput.outputs.reduce((sum, o) => (o.address === address ? sum + o.amount : sum), 0);
    ...
    return tempestiveTransactions.reduce((acc, t) => {
      if (t.input.timestamp > mostRecentTimestamp) {
        return (
          acc +
          t.outputs.reduce((sum, o) => (o.address === address ? sum + o.amount : sum), 0)
        );
      }
